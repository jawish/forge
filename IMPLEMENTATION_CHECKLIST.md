...
- [ ] **7.2 First per-repo images**: one Python + one TS repo, Chainguard base, `.forge/config.toml` + `.forge/setup.sh` valid per `repoConfigSchema`; build → cosign sign → GHCR. Snapshot+restore verified on dev account. *(Python + TS fixtures built + config-validated against repoConfigSchema. Full build pipeline validated end-to-end locally: both images build on Chainguard `-dev` base (non-root 65532), app tests pass (pytest/vitest), CycloneDX SBOMs generated, Trivy HIGH/CRITICAL gate clean, cosign sign→verify verified, negative test (wrong key) correctly fails. The GHCR push + CF Sandbox snapshot/restore need the dev CF account — §6.1 runbook.)*
